...
public:
	...
	virtual void on_collide()  //是否发生碰撞
	{
		if (callback)
			callback();
	}

	//子弹碰撞的判定是子弹矩形中心的坐标是否进入玩家角色矩形内部而非两者边框的相交
	//所以此处+传入的参数应当是玩家的坐标和尺寸
	...
	virtual void on_update(int delat){}
...
protected:
	...
	//对于每个子弹，有三个阶段，射出，命中（碰撞播放动画），移除（防止再次碰撞影响更新）
