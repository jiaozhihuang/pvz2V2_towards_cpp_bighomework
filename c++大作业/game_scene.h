...
class GameScene : public Scene//游戏局内场景
{public:
	GameScene() = default;
	~GameScene() = default;
	void on_enter() //场景开始
	{   
		...
		large_platform.render_position.y = 422;
	...
	void on_draw(const Camera& camera) // 场景渲染
	{
		...
		if (is_debug)
		{
			...
			outtextxy(15, 15, _T("已开启调试模式,'Q'键关闭"));
		} 
	...
	void on_input(const ExMessage& msg)//用户输入
	{
 ...
 private:
	 ...
	 Timer timer_winner_slide_in;  //计算动效果划入划出计时器
