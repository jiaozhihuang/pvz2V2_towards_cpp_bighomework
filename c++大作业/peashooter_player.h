...
public:
	PeashooterPlayer(bool facing_right = true) : Player(facing_right)
	{
		...
		animation_attack_ex_left.set_interval(150);
		animation_attack_ex_right.set_interval(150);
	...
	void on_attack()
	{
		...
		switch (rand() % 2)
		{
		case 0:mciSendString(_T("play pea_shoot_1 from 0"), NULL, 0, NULL); break;
		case 1:mciSendString(_T("play pea_shoot_2 from 0"), NULL, 0, NULL); break;
		}
...
private:
	void spawn_pea_bullet(float speed)    //豌豆子弹的发射逻辑
	{
		...
		//豌豆子弹从头部射出逻辑上比较合理
		const Vector2& bullet_size = bullet->get_size();    //获取豌豆射手尺寸
		bullet_position.x = is_facing_right					//检查朝向并安排子弹射出位置
		...
		//push_back函数的作用是将元素放置于容器的末位，如图这里是放在bullet的动态数组里
...
private:
	...
	Timer timer_spawn_pea_ex; //豌豆子弹发射计时器
