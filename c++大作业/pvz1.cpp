...
Atlas atlas_peashooter_idle_right;      // 豌豆射手向右的默认动画图集
...
void flip_atlas(Atlas& src, Atlas& dst)//两个参数分别表示反转前后的目标图片
{   
	dst.clear();
	for (int i = 0; i < src.get_size(); i++)//遍历图集中每一张/帧图片
	{
		IMAGE img_flipped;
...
void load_game_resource()//加载游戏字体、加载处理游戏图片素材、加载游戏音乐
{
...
{
	ExMessage msg;
	const int FPS = 60;//限制游戏帧率

	load_game_resource();

	initgraph(1280, 720,EW_SHOWCONSOLE);//场景尺寸,初始化绘图窗口
	settextstyle(28, 0, _T("IPix"));
	setbkmode(TRANSPARENT);
	BeginBatchDraw();

	//默认构造函数产生对象，new为对象动态开辟内存
	//由于c++多态的规则
	// 基类指针可以指向用new为派生类动态开辟的空间，但是该指针只能访问继承自基类的成员
	...
	//游戏本质是不断重复的死循环，根据用户的输入进入不同场景，实现功能
	while (true)
	{
